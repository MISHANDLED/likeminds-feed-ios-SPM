...
public class LMFeedBasePostDetailViewModel {
    ...
    public init(postID: String, delegate: LMFeedBasePostDetailViewModelProtocol?, openCommentSection: Bool, scrollToCommentSection: Bool) {
        self.postID = postID
        self.commentList = []
        self.currentPage = 1
        self.pageSize = 20
...
public extension LMFeedPostDetailViewModel {
    ...
    func handleDeleteComment(for commentID: String) {
        ...
        if comment.userDetail.userUUID == LocalPreferences.userObj?.sdkClientInfo?.uuid {
            let alert = UIAlertController(title: LMStringConstants.shared.deleteComment, message: LMStringConstants.shared.deleteCommentMessage, preferredStyle: .alert)
